...
25次运行的原始JSON、best checkpoint、共享初始backbone和run manifest均已提交。最初20次W2/W3历史文件继续保持原字节不变；A3配置勘误见[`experiments/v2/w3/A3/ERRATA.json`](experiments/v2/w3/A3/ERRATA.json)，A2-L汇总判定勘误见[`experiments/v2/w5/SUMMARY_ERRATA.json`](experiments/v2/w5/SUMMARY_ERRATA.json)。当前169项离线自动化测试通过；发布结论仍以最终纠正版审计为准。
...
当前169项离线测试不需要RadioML数据、GPU或API密钥：

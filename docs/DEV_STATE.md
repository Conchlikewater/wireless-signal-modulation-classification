...
- 独立 `.venv` 当前没有 `pytest`；W0 尝试运行离线测试时在收集前停止，因此历史84项测试本次没有重跑。W1 修改代码前必须先确认测试执行环境，安装依赖仍需用户批准。
...
2. [x] 旧 V2 文档已分诊并单独提交；
3. [x] 单次训练耗时已确认；
4. [x] W0 学习讲解和三道具体问题已完成；
5. [x] 本文件的“⑤ 拥有权验证”已改为“已完成”；
6. [ ] 用户明确发出 W1 开工指令。
...
- W1 写第一行业务代码前应先解决可审计的 pytest 执行入口，否则新增测试无法可靠验收。
